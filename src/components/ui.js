...
class Renderer {
  ...
  renderBoards(user, computer, hasStarted) {
    ...
    if (hasStarted) {
      this.addUserAttackEventListeners(
        user,
        computer,
  ...
  addUserShips(user, computer) {
    ...
    for (let i = 0; i < cells.length; i++) {
      cells[i].addEventListener("mouseover", () => {
        this.renderText(
          "Place your " + sizes[cur][0] + "! (Press R to rotate)",
        );

        for (let j = 0; j < coordinates.length; j++) {
          cells[coordinates[j][1] * 10 + coordinates[j][0]].classList.remove(
            "preview",
          );
        }
        coordinates = [];
        ...
        for (let j = 0; j < coordinates.length; j++) {
          cells[coordinates[j][1] * 10 + coordinates[j][0]].classList.add(
            "preview",
          );
        }
      });
  ...
  addUserAttackEventListeners(user, computer, computerBoardDiv) {
    ...
    for (let i = 0; i < cell.length; i++) {
      if (
        !cell[i].classList.contains("missed") &&
        !cell[i].classList.contains("hit")
      ) {
          ...
          () => {
            ...
            setTimeout(() => {
              ...
              if (result.hit) {
                ...
                if (result.ship.isSunk()) {
                  message =
                    computer.name + " has sunk the " + result.ship.name + "!";
                } else {
  ...
  showShips(gameboard, boardDiv) {
    ...
    for (let i = 0; i < gameboard.ships.length; i++) {
      for (let j = 0; j < gameboard.ships[i].position.length; j++) {
        ...
        if (
          !cells[y * 10 + x].classList.contains("hit") &&
          !cells[y * 10 + x].classList.contains("missed")
        ) {
